...
export async function buildFixture({port=0,delayMs=2500}={}){
  ...
  const runtime=createLocalCmsRuntime({
    ...
    imageGenerator:{...mock,async generateImage(input){calls++;await new Promise(r=>setTimeout(r,delayMs));return mock.generateImage(input);}},
