...
function imagePromptFor(request) {
  ...
  if (task === 'projectile-sprite') {
    return [
      ...
      '',
      'Format:',
      '- ONE projectile object, centered, filling most of the frame but with a generous magenta margin on all sides — nothing touches the edge.',
      '- No character, no frame strip, no grid, no shadow, no text, no UI.',
      '- The projectile points/travels to the RIGHT.',
      '- Crisp, readable silhouette; one continuous shape (effects/trails stay attached).',
      '- Solid chroma-magenta background (#ff00ff), not transparency or scenery.',
      '',
