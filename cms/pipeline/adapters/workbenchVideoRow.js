...
async function generateExclusive({characterId,moveId,prompt,task,storage,repository,onProgress,onGenerationAttempt,provider='fal'}) {
  ...
  const actorId=draft.motionActors?.[moveId]??draft.moves?.find(m=>m.id===moveId)?.controlledActor;
  const referenceSprite=actorId?draft.actors?.find(a=>a.id===actorId)?.sprite:draft.sprite;
  const referenceFile=referenceSprite?.frames?.base?.[0]?.file??'sprites/base/base_001.png';
  ...
  const motionPrompt=artStyle==='paint'
    ? `Animate only this exact painted ${actorId?'summoned entity':'nonhuman creature'}. ${draft.motionPrompts?.[moveId]??profile.prompt} ${characterArtDirection(artStyle)} Fixed side camera, animate in place; engine supplies translation. Preserve palette and identity. Background stays uniform white. Entire subject visible with generous margins.`
