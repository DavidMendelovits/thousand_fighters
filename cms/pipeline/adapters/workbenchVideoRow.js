...
async function generateExclusive({characterId,moveId,prompt,task,storage,repository,onProgress,onGenerationAttempt,provider='fal'}) {
  ...
  const artStyle=draft.artStyle==='watercolor'?'watercolor':'pixel';
  ...
  if(!directory){
    ...
    await exec('python3',['scripts/prepare_animation_reference.py',path.join(directory,'sprite.png'),'--output',path.join(directory,'reference.png'),'--size','768','--occupancy','0.65']);
  ...
  const job=await runVideoJob(
    resume?{resume:directory}:{provider,mode:'image-to-video',image:path.join(directory,'reference.png'),...(provider==='pruna'?{recipe:'quality',...(profile.loop?{'end-image':path.join(directory,'reference.png')}:{})}:{}),prompt:motionPrompt,duration:provider==='pruna'?5:profile.duration,output:directory},
    {
  ...
  if(!exists)await storage.lineage.run({characterId,stage:'compile-motion',moveId,inputs:{video:job.archivedOutput,reference:await storage.lineage.artifact(bytes,{contentType:'image/png'}),style:artStyle,frames:profile.frames,loop:profile.loop,compiler:await storage.lineage.artifact(await readFile('scripts/compile_character_motion.py'),{contentType:'text/x-python'})}},()=>exec('python3',['scripts/compile_character_motion.py',path.join(directory,'source.mp4'),'--reference',path.join(directory,'sprite.png'),'--output',compiled,'--action',moveId,'--style',artStyle,'--frames',String(profile.frames),...(profile.loop?['--loop']:[])],{timeout:120000,maxBuffer:2*1024*1024}));
