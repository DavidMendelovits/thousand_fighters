...
export function convertDraftToCharacterConfig({ draft, frameData, manifest: rawManifest }) {
  ...
  return {
    ...
    comboRoutes: (draft.combos??[]).map(combo=>({name:combo.displayName??combo.id,moves:combo.segments,purpose:combo.purpose??'Authored move sequence. Preview spacing and cancel windows in the workbench.'})),
    ...
    jumpBackVelocity: Math.abs(stats.jumpBackVelocity ?? 2.8),
