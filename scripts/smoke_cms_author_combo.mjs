 ...
 *   [B] occupied legacy rows are never reused: new combo-exclusive moves get
 *       stable custom rows without clobbering existing move animations.
 *
...
try {
  ...
  // ---------------------------------------------------------------------------
  console.log('\n[B] all legacy rows occupied → a new dedicated combo row');
  {
    ...
    test('new move receives a stable exclusive row, even with every legacy row occupied', () => {
      assert.equal(result.createdMoves.length, 1);
      assert.equal(result.createdMoves[0].animation,'combo_overflow_2');
      assert.equal(result.createdMoves[0].trigger.cancelOnly,true);
      assert.ok(!result.warnings.some((w) => /no free animation rows|reuses/.test(w)), `unexpected row reuse: ${JSON.stringify(result.warnings)}`);
    });
    test('did NOT regenerate any owned row (no clobber)', () => {
      assert.equal(result.spriteResults.length, 1, 'only the new exclusive row is generated');
      assert.equal(result.spriteResults[0].moveId,'combo_overflow_2');
      for(const original of moves)assert.equal(saved.moves.find(move=>move.id===original.id).animation,original.animation);
    });
