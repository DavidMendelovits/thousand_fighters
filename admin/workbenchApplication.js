...
export function mountWorkbenchApplication(session) {
...
session.listen(window,'message', event => {
...
if(benchmarkLink){
  ...
  session.listen(benchmarkLink,'click',event=>{event.preventDefault();navigateTo('/pipeline');});
}
...
function handleRouteChange() {
  ...
  if (route.page === 'pipeline') {
    ...
    session.documentScope.own(mountBenchmarks({host:document.getElementById('pipeline-benchmarks')}));
    setOpsTab('pipeline');
  } else if (route.isNew) {
...
session.listen(window,'popstate', () => {
  handleRouteChange();
});
...
session.listen(elements.draftForm,'submit', async (event) => {
  event.preventDefault();
  await createDraft();
});
session.listen(elements.runChain,'click', runLocalE2E);
session.listen(elements.generateSheet,'click', generateSheet);
session.listen(elements.normalizePack,'click', normalizePack);
session.listen(elements.validatePack,'click', validatePack);
session.listen(elements.publishCharacter,'click', publishCharacter);
session.listen(elements.assetUploadForm,'submit', uploadSpriteAsset);
session.listen(elements.assetKind,'change', syncAssetPath);
session.listen(elements.assetMove,'change', syncAssetPath);
session.listen(elements.assetActor,'input', syncAssetPath);
session.listen(elements.assetFrame,'input', syncAssetPath);
session.listen(elements.assetPath,'input', () => {
  if (elements.assetKind.value === 'custom') return;
  syncAssetPath();
});
session.listen(elements.assetFile,'change', syncAssetPath);
session.listen(elements.characterWorkbench,'click', handleAnimationControl);
session.listen(elements.characterWorkbench,'click', handleWorkbenchClick);
// Per-row prompt edits live in state so workbench re-renders (e.g. while other
// rows finish generating) don't wipe them.
session.listen(elements.characterWorkbench,'input', (event) => {
...
session.listen(elements.characterWorkbench,'change', event => {
...
session.listen(elements.chatForm,'submit', sendChatMessage);
session.listen(elements.refreshRoster,'click', async () => {
  await loadCharacters();
  handleRouteChange();
});
session.listen(document.querySelector('.new-fighter-link'),'click', (event) => {
  event.preventDefault();
  navigateTo('/roster/new');
});
session.listen(document.getElementById('roster-search'),'input', event => { state.rosterQuery = event.target.value; renderLibrary(); });
session.listen(document.getElementById('roster-filters'),'click', event => {
  const button = event.target.closest('[data-collection]');
  if (button) { state.rosterFilter = button.dataset.collection; renderLibrary(); }
});
session.listen(elements.opsTabs,'click', (event) => {
  const tab = event.target.closest('[data-ops-tab]');
  if (tab) setOpsTab(tab.dataset.opsTab);
});
session.listen(elements.adapterHealthStrip,'click', () => setOpsTab('pipeline'));

// Delegated click on chat thread: open error modal when an error tool call summary is clicked
session.listen(elements.chatThread,'click', (event) => {
  const errorRow=event.target.closest('.run-log-error-line');
  if(errorRow){openErrorModal({name:'run-log',status:'error',input:null,result:null,error:errorRow.dataset.errorMessage});return;}
...
session.listen(elements.characterList,'click',event=>{
  const button=event.target.closest('[data-character-id]');
  if(button)void selectCharacter(button.dataset.characterId);
});

session.actions = {selectCharacter,navigateTo,renderCharacterWorkbench,renderEmptyWorkbench,openTestbed,openGym};
session.ready = boot();
...
async function selectCharacter(characterId, options = {}) {
  // A completed mutation may refresh its own document, never navigate back
  // after the user has selected a different fighter or the new-fighter page.
  if(session.disposed||(options.silent&&(!state.studio||state.currentCharacterId!==characterId)))return;
  if(state.preview && !state.preview.canLeave())return;
  const isCurrent=session.beginSelection();
  ...
  if (!isCurrent() || seq !== state.selectSeq || state.currentCharacterId !== characterId) return;
  ...
  try {
    ...
    if (state.currentCharacterId === characterId) {
      ...
      if (successCard) {
        ...
        session.documentScope.timeout(() => successCard.classList.remove('move-card-success'), 2000);
      }
    }
...
async function beginMotionReview(action){
  ...
  const reviewScope=session.documentScope.replaceScope('motionReview');
  reviewScope.listen(panel.querySelector('form'),'submit',async event=>{
    ...
    try{
      ...
      if(reviewScope.disposed)return;
      await selectCharacter(characterId,{silent:true});
...
// Durable submission + shared stream/poll recovery. The server owns extraction.
async function invokeToolStreaming(name, input, onProgress) {
  return runBuildJob({tool:name,input,onProgress,signal:session.documentScope.signal});
}
...
function renderNewFighterWorkbench() {
  elements.characterWorkbench.inert=false;
  clearAnimationTimers();
  session.replaceDocument();++state.selectSeq;++session.selection;
  state.preview=null;state.studio=null;
  ...
  session.documentScope.listen(document.getElementById('new-fighter-form'),'submit', onCreateNewFighter);
  session.documentScope.listen(document.getElementById('import-published-form'),'submit',async event=>{
  ...
  session.documentScope.listen(document.getElementById('new-fighter-ref'),'change', onNewFighterRefImage);
}
...
function renderCharacterWorkbench(draft, assets) {
  clearAnimationTimers();
  const scope=session.replaceDocument();
  ...
  session.documentScope.listen(previewHost,'change',event=>{if(event.target.matches('[data-preview-row]'))document.getElementById('motion-review').hidden=true;});
  session.documentScope.listen(previewHost,'click',event=>{if(event.target.closest('[data-preview-testbed],[data-preview-close]'))document.getElementById('motion-review').hidden=true;});
  state.preview = scope.own(mountPreview({host: previewHost, detail:previewDetail, gameBase: TESTBED_BASE_URL}));
  state.studio=scope.own(mountWorkbenchLayout({host:elements.characterWorkbench,characterId:draft.id,groups:moveGroups,preview:state.preview}));
  startAnimationPreviews();
  mountCharacterComponents({scope,host:document.getElementById('character-components'),draft,invoke:(name,input)=>name==='get_animation_plan'?postJson(`/api/tools/${name}`,input).then(r=>r.result):invokeTool(name,input),
  ...
  mountCharacterHistory({scope,host:document.getElementById('character-history'),characterId:draft.id,getJson,postJson,onRestore:async()=>{await selectCharacter(draft.id,{silent:true});const panel=document.getElementById('character-history');panel.open=true;panel.scrollIntoView({block:'start'});}});
  scope.own(mountBuildJobs({host:document.getElementById('character-build-jobs'),characterId:draft.id,
    ...
    onHistory:scope.guard(()=>{state.studio.choose('history');const panel=document.getElementById('character-history');panel.open=true;panel.scrollIntoView({block:'start'});})}));
  scope.own(mountBuildPlans({scope,host:document.getElementById('character-build-plans'),characterId:draft.id,
    onReload:scope.guard(async()=>{await selectCharacter(draft.id,{silent:true,pushState:false});document.getElementById('character-build-plans')?.scrollIntoView({block:'start'});})}));
}
...
function startAnimationPreviews() {
  ...
  for (const player of elements.characterWorkbench.querySelectorAll('[data-animation-player]')) {
    ...
    const timer = session.documentScope.interval(() => {
...
function openErrorModal(toolCall) {
  ...
  const modalScope=session.replaceScope('errorModal');
  modalScope.listen(closeButton,'click', closeErrorModal);

  const diagnoseButton = elements.errorModal.querySelector('.error-modal-diagnose');
  modalScope.listen(diagnoseButton,'click', diagnoseWithCodex);

  modalScope.listen(elements.errorModal,'click', handleModalBackdropClick);
...
function closeErrorModal() {
  session.replaceScope('errorModal');
...
function openMoveActivityPanel(moveId) {
  ...
  `;
  session.replaceScope('moveActivity').listen(elements.moveActivityPanel.querySelector('.move-activity-close'),'click', closeMoveActivityPanel);
  elements.moveActivityPanel.hidden = false;
}

function closeMoveActivityPanel() {
  session.replaceScope('moveActivity');
...
function showError(error) {
  ...
  if(!notice){
    ...
    session.listen(notice.querySelector('button'),'click',()=>{notice.hidden=true;});document.body.append(notice);
  }
