...
export function renderMotionReprocess(draft, action) {
  ...
  return `<details class="motion-reprocess" data-motion-reprocess="${escape(action)}"><summary>Reprocess saved video · no generation</summary>
    ...
    <label><input data-reprocess-expand type="checkbox" ${(row.provenance?.options?.expandCanvas??['paint','watercolor'].includes(draft.artStyle))?'checked':''}> Expand canvas to contain the full silhouette</label><p>Preserves oversized effects instead of cropping them. Check the scale and anchor in gameplay after reprocessing.</p>
    ${draft.artStyle!=='paint'&&draft.artStyle!=='watercolor'?`<label>Background key<select data-reprocess-background aria-label="${escape(action)} Background key">${[['pruna-frame','Per-frame key · background gaps'],['auto-frame','Per-frame key · exterior only'],['magenta-distance','Fixed magenta distance'],['magenta','Fixed magenta chroma']].map(([value,label])=>`<option value="${value}" ${value===(row.provenance?.options?.background??'magenta')?'selected':''}>${label}</option>`).join('')}</select></label><p>Use per-frame key if the saved provider video switches between magenta and white. Inspect enclosed highlights and any color bleed before approval.</p>`:''}
    ...
    ${draft.artStyle!=='paint'&&draft.artStyle!=='watercolor'?`<label><input data-reprocess-despill type="checkbox" ${row.provenance?.options?.despillMagenta?'checked':''}> Clean exposed magenta video fringe</label><p>Only exposed pink pixels are removed; inspect thin ribbons and lantern edges before accepting.</p>`:''}
    ${draft.artStyle!=='paint'&&draft.artStyle!=='watercolor'?`<label><input data-reprocess-magenta-gaps type="checkbox" ${row.provenance?.options?.keyMagentaGaps?'checked':''}> Remove magenta islands inside white-key frames</label><p>Use only if this actor has no intentional pink or purple details. Reprocesses the archived video without a provider call; inspect every pose before approval.</p>`:''}
...
export function readReprocessControls(host) {
  ...
  return { frames, start, end, loop, expandCanvas:Boolean(host.querySelector('[data-reprocess-expand]')?.checked), matteCleanup:Boolean(host.querySelector('[data-reprocess-matte]')?.checked), refineEdges:Boolean(host.querySelector('[data-reprocess-refine]')?.checked), despillMagenta:Boolean(host.querySelector('[data-reprocess-despill]')?.checked), keyMagentaGaps:Boolean(host.querySelector('[data-reprocess-magenta-gaps]')?.checked), ...(host.querySelector('[data-reprocess-background]')?{background:host.querySelector('[data-reprocess-background]').value}:{}), ...(hasContact?{contactFrame,recoveryFrame}:{}) };
}
